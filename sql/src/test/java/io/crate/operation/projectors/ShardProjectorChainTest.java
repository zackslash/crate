...
public class ShardProjectorChainTest extends CrateUnitTest {
    ...
    private static void assertRowReceiver(RowReceiver rowReceiver, Class<?> expectedClass) {
        if (rowReceiver instanceof PassThroughRowMergers.MultiUpstreamRowReceiver) {
            assertThat(((PassThroughRowMergers.MultiUpstreamRowReceiver) rowReceiver).delegate, instanceOf(expectedClass));
        } else {
