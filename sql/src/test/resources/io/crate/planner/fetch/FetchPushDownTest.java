...
public class FetchPushDownTest {
    ...
    public void testLimitIsPushedDown() throws Exception {
        ...
        assertThat(sub.limit(), is(10));
        assertThat(sub.offset(), is(100));

        assertThat(qs.limit(), is(10));
        assertThat(qs.offset(), is(0));
    }
    ...
    public void testPushDownWithoutOrder() throws Exception {
        ...
        assertThat(qs.outputs().get(0).symbolType(), is(SymbolType.FETCH_REFERENCE));
        assertThat(qs.outputs().get(1).symbolType(), is(SymbolType.FETCH_REFERENCE));

        FetchReference ref = (FetchReference) qs.outputs().get(0);
        assertThat(ref.docId(), instanceOf(InputColumn.class));
        assertThat(ref.referenceIdent(), is(REF_A.ident()));

        ref = (FetchReference) qs.outputs().get(1);
        assertThat(ref.referenceIdent(), is(REF_I.ident()));

        assertThat(sub.outputs(), hasSize(1));
        assertThat(sub.outputs().get(0).symbolType(), is(SymbolType.REFERENCE));
        Reference docId = (Reference) sub.outputs().get(0);
        assertThat(docId, isReference("_docid"));
    }
    ...
    public void testPushDownWithOrder() throws Exception {
        ...
        assertThat(qs.outputs(), hasSize(2));
        assertThat(qs.outputs().get(0).symbolType(), is(SymbolType.FETCH_REFERENCE));
        assertThat(qs.outputs().get(1).symbolType(), is(SymbolType.INPUT_COLUMN));

        FetchReference ref = (FetchReference) qs.outputs().get(0);
        assertThat(ref.docId(), instanceOf(InputColumn.class));
        assertThat(ref.referenceIdent(), is(REF_A.ident()));

        InputColumn col = (InputColumn) qs.outputs().get(1);
        assertThat(col.index(), is(1));
